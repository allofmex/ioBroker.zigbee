...
class DeviceConfigure extends BaseExtension {
    ...
    onDeviceJoin(device) {
        this.debug(`DeviceConfigure at (re-)join ${device.ieeeAddr} ${device.modelID}`);
        if (device.meta.hasOwnProperty('configured')) {
            // invalidate stored configureKey to trigger configure run in next onZigbeeEvent
        ...
        try {
            ...
            try {
                ...
                this.error(
                    ...
                    `attempt ${this.attempts[device.ieeeAddr] + 1}`,
                );
                this.debug(`DeviceConfigure failed ${device.ieeeAddr} with ${error.stack}`);
